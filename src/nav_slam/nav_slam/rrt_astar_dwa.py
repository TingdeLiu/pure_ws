...
class NavigationControl(Node):
    def __init__(self):
        ...
        self.img_path = os.path.expanduser('~/rrt_path_snapshot.png')
        self.frames_dir = '/tmp/rrt_frames/'
        os.makedirs(self.frames_dir, exist_ok=True)
        self.frame_count = 0

        self.create_timer(0.5, self.save_frame)
    ...
    def save_frame(self):
        ...
        plt.axis('equal')
        plt.grid(True)
        frame_file = os.path.join(self.frames_dir, f'frame_{self.frame_count:04d}.png')
    ...
    def map_callback(self, msg):
        ...
        # 将三层膨胀层与障碍分级处理
        # grid[(grid >= -2) & (grid < 50)] = 1      # 空地
        # grid[(grid >= 50) & (grid < 90)] = 50    # 第一层膨胀：轻微危险（可通行）
        # grid[(grid == 90)] = 99    # 第二层膨胀：接近不可通行
        # grid[(grid >= 91)] = 100    # 第三层膨胀：不可通行
        ...
        self.send_path_to_nav2(path_smooth)
    ...
    def send_path_to_nav2(self, path_coords):
        path_msg = Path()
        ...
        self.path_publisher2.publish(path_msg)
